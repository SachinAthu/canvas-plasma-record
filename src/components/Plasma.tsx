...
import type { RecordingOptions } from "./RecordingForm"
...
export const Plasma: React.FC<PlasmaProps> = ({
  ...
  const startRecording = (options: RecordingOptions) => {
    ...
    recorder.onstop = () => {
      const blob = new Blob(chunks, { type: options.format.split(";")[0] })
      const url = URL.createObjectURL(blob)
      const fileExtension = options.format.startsWith("video/mp4")
        ? "mp4"
        : "webm"

      const a = document.createElement("a")
      a.href = url
      a.download = `plasma_recording.${fileExtension}`
